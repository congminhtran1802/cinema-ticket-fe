      ...
      <div className='grid grid-cols-12'>
        <span className='col-span-9 mt-5'>
          <div className='flex flex-col items-center'>
            <div className='flex bg-black h-4 relative justify-center' style={{ width: '80%' }}><span className='absolute top-7'> {t('screen') }</span></div>
            ...
            <table className=" divide-y divide-gray-200 w-2/3">
              <thead className="bg-gray-50 p-5">
                <tr>
                  ...
                  <th>{t('VIPSeats') }</th>
                  <th>{t('ReservedSeats') }</th>
                  <th>{t('SeatsYouveReserved') }</th>
                  <th>{t('SeatsBeingReservedby') }</th>
                </tr>
              </thead>
        ...
        <span className='col-span-3 mt-5'>
          ...
          <p>{t('location') } {thongTinPhim.tenCumRap} - {thongTinPhim.tenRap}</p>
          <p>{t('showdate') }: {thongTinPhim.ngayChieu} - {thongTinPhim.gioChieu}</p>
          <hr />
          <div className='flex flex-row my-4'>
            <div className='w-11/12'>
              <span className='text-red-600 text-lg'>{t('seat') }</span>
          ...
          <div className='h-full flex flex-col items-center' >
            ...
            }} className='mb-0 bg-green-500 text-white w-full text-center font-bold text-2xl py-3 cursor-pointer'> {t('book_tickets') }</div>
          </div>
        </span>
      </div>
    </div>
        ...
        return <div className="p-2 lg:w-1/3 md:w-1/2 w-full" key={index}>
            <div className="h-full flex items-center border-gray-200 border p-4 rounded-lg">
                ...
                <div className="flex-grow">
                    ...
                    <p><span className="font-bold">{t('location') }</span> {seats.tenHeThongRap}   </p>
                    <p>
                        <span className="font-bold">{t('namebranch') }</span>  {seats.tenCumRap} - <span className="font-bold">{t('seat') }:</span>  {ticket.danhSachGhe.map((ghe, index) => { return <span className="text-green-500 text-xl" key={index}> [ {ghe.tenGhe} ] </span> })}
                    </p>
                </div>
            </div>
        </div>
